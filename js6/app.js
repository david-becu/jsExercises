...
function createNode(elementParent, tag, classAttribute, contenuHTML) {
  ...
  if (typeof classAttribute !== 'undefined') {
    if (typeof classAttribute == 'text') {

    } else {
      for (i = 0; i <= classAttribute.length - 1; i++) {
        elem.className = classAttribute[i];
        elem.setAttribute(classAttribute[i][0], classAttribute[i][1]);
      }
    }
...
createNode('body', 'div', [['class'], ['container']], '');
